...
// similar to Cell, RefCell also implments
// !Sync implicitly by using UnsafeCell
...
impl<T> std::ops::Deref for Ref<'_, T> {
    ...
    fn deref(&self) -> &Self::Target {
        // SAFETY: a Ref is only created when no exclusive references have been given out.
        // once it is given out, state is set to shared and no exclusive references are given out.
...
impl<T> std::ops::DerefMut for RefMut<'_, T> {
    fn deref_mut(&mut self) -> &mut Self::Target {
        // SAFETY: a RefMut is only created when no other references have been given out.
        // once it is given out, state is set to exclusice and no other references are given out.
